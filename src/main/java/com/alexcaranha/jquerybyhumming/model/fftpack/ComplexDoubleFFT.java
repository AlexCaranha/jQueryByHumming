...
*/
     ...
     /**
        * Backward complex FFT transform. It is the normalized inverse transform of <em>ft</em>(Complex1D[]).
        *
        *
        * @param data  an array of <em>n</em> Complex data
     */
     public void btNormalized(Complex1D data, int ncasas)
     {
         if(data.x.length != ndim)
              throw new IllegalArgumentException("The length of data can not match that of the wavetable");
         double[] y = new double[2*ndim];
         ...
         for(int i=0; i<ndim; i++)
         {
              data.x[i] = arredondar(y[2*i]/ndim, ncasas);
              data.y[i] = arredondar(y[2*i+1]/ndim, ncasas);
         }
     }
