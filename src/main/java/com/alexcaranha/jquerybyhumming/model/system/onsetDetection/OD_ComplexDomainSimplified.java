 ...
 * @author alexcaranha
 */
public class OD_ComplexDomainSimplified extends OD {

    public OD_ComplexDomainSimplified(int sampleRate, int frameLength, double overlap,
    ...
    public SignalXY processing(SignalXY signalXY) throws Exception {
        ...
        for (iFrame = 2, dataBuffer = new Complex1D(frameLength);
            ...
            //------------------------------------------------------------------
            Complex1D   dataFFT_aux = dataFFT[iFrame];
            double[]    phase = phases[iFrame];
            for (int i = 0; i < frameLength; i += 1) {
                double b = dataBuffer.x[i];
                double c = dataFFT_aux.x[i];

                // lei dos cossenos.
                value += Math.sqrt(b * b + c * c - 2 * b * c * Math.cos(phase[i]));
            }
            //------------------------------------------------------------------
    ...
    public String getTitle() {
        return "COMPLEX DOMAIN SIMPLIFIED";
    }
    ...
    public static void main(String[] args) throws Exception {
        OD od = (OD_ComplexDomainSimplified) App.getContext().getBean("od_complexDomainSimplified");
        if (od != null) {
