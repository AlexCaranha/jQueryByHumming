 ...
 * @author alexcaranha
 */
public class OD_RectifiedComplexDomain extends OD {

    public OD_RectifiedComplexDomain(int sampleRate, int frameLength, double overlap,
    ...
    public SignalXY processing(SignalXY signalXY) throws Exception {
        ...
        double[]    distances = new double[frameLength];
        ...
        for (iFrame = 2, dataBuffer = new Complex1D(frameLength);
            ...
            //------------------------------------------------------------------
            dataBuffer.distancesAbs(dataFFT[iFrame], distances);
            //------------------------------------------------------------------
            value = 0.0;
            for (int i = 0; i < frameLength; i++) {
                value += (modules[iFrame][i] > modules[iFrame - 1][i])
                            ? distances[i]
                            : 0.0;
            }
            //------------------------------------------------------------------
    ...
    public static void main(String[] args) throws Exception {
        OD od = (OD_RectifiedComplexDomain) App.getContext().getBean("od_rectifiedComplexDomain");
        if (od != null) {
